...
class ChessboardCalibration(Debugable):
  ...
  def __init__(self, debug=False):
    ...
    # print('frame resolution: {}'.format(self.__out_size))                                                                         # COMMENTED OUT OUTPUT
  ...
  def mapping(self, chessboard_img=None, fix_rotate=False, rotate_val=-90, add_padding=False, apply_kdilate=True, smooth_ksize=(11, 11)):
    ...
    if len(pa_image) == 0: return None                                                      # ADDED NEW EXIT
    ...
    if corners:
      ...
      chess_matrix = self.parseMatrix(self.__matrix)                                                    # ADDED NEW VARIABLE TO SEPARATE PARAMETER AND FUNCTION CALL
      if chess_matrix == None: return None                                            # ADDED NEW EXIT TO PREVENT AN EXCEPTION FROM HAPPENING WHICH ENDS EXECUTION
    #   print('playable!')
      self.board.addSquares(chess_matrix)                                            # CALLING NEW VARIABLE INSTEAD OF FUNCTION DIRECTLY
  ...
  def playableArea(self, chessboard_image):
    ...
    self.save('1.9_before_countour.jpg', thresh)

    self.board.contours = self.biggestContours(thresh)
    if len(self.board.contours) == 0: print("FALIED NO CONTOURS"); return []   
    # if len(self.board.contours) != 4: print("FALIED CONTOUR: Degree not 4"); return []                                                # ADDED CHECK FOR POLYGON DEGREE
    # elif not cv2.isContourConvex(self.board.contours): print("FALIED CONTOUR: Concave"); return []                                                # ADDED CHECK FOR POLYGON CONVEXITY
  ...
  def biggestContours(self, thresh):
    ...
    cnts = sorted(cnts, key=cv2.contourArea, reverse = True)
                                                                                                    ...
                                                                                                    # ADDED TO ONLY SELECT COUNTOURS WITH DEGREE 4 AND CONVEX
    print('Before: ' + str(len(cnts)))
    while len(cnts) > 0 and (len(cv2.approxPolyDP(cnts[0], 0.025 * cv2.arcLength(cnts[0], True), True)) != 4 or not cv2.isContourConvex(cv2.approxPolyDP(cnts[0], 0.025 * cv2.arcLength(cnts[0], True), True))):
      cnts.pop(0)
    print('After: ' + str(len(cnts)))
    
    if len(cnts) == 0: return []                                                                # ADDED TO AVOID INDEX OUT OF RANGE
  ...
  def parseMatrix(self, matrix):
    ...
    if len(matrix) != 9:
      ...
      return None                                                                           # CHANGED TO RETURN None instead of literally nothing before (just "return")  
    else:                                                                                   # ADDED CHECK FOR ROWS
      for row in matrix: 
        if len(row) != 9:
          print("[ERROR] No 9x9 dimension matrix found")
          return None    
  ...
  def saveMapping(self):
    ...
    """
    if self.board.contours is None:
      print('Cannot find a contours')                                             # COMMENTED OUT EXCEPTION AND PRINTS IT AND RETURNS NONE INSTEAD
      return 1
      # raise Exception('Cannot find a contours')

    if self.board.squares is None:
      print('Cannot find a squares')                                             # COMMENTED OUT EXCEPTION AND PRINTS IT AND RETURNS NONE INSTEAD
      return 1
      # raise Exception('Cannot find a squares')

    if len(self.__matrix) == 0:
      print('No 8x8 matrix found to save')                                             # COMMENTED OUT EXCEPTION AND PRINTS IT AND RETURNS NONE INSTEAD
      return 1
  ...
  def loadMapping(self, rotateBool=False) -> Tuple[bool, Board]:                                                           # Added bool var
    """
    ...
    rotate_val = dictionary['rotate_val']                                                                                     # Added new temp var
    if rotateBool:
      print("Fix rotation? Enter number of 90˚ clockwise rotations: ", end='')                                                 # ADDED USER PROMPT TO FIX ROTATION
      value = input()
      if str(value) == '1': rotate_val+=90;print('90')
      elif str(value) == '2': rotate_val+=180;print('180')
      elif str(value) == '3': rotate_val+=270;print('270')
    self.rotate_val = rotate_val
