...
class Camera:
    ...
    def __init__(self, cam_address, resize_percent=0.35, fps=30):
        ...
        self.fps = fps
        self.cap = cv2.VideoCapture(cam_address)  # Updated to use cam_address passed to the constructor

        if self.cap.isOpened():
            width = self.cap.get(cv2.CAP_PROP_FRAME_WIDTH)
            height = self.cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
            self.resolution = (height, width)  # Correct resolution order (height, width)
        else:
    ...
    def startRunning(self, callback):
        ...
        **OBS**
        This method locks execution of the next lines of the caller
        """
        self.stop = False
        while True:
            ...
            # The interval is 1s divided by number of frames
