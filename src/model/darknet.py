...
class Darknet:
  ...
  def __init__(self):
    ...
    unconnected_layers = self.__net.getUnconnectedOutLayers()
    # Handle different return types across OpenCV versions
    if len(unconnected_layers.shape) == 1:
        # OpenCV >= 4.x on some platforms
        self.__out_layers = [self.__out_layers[i - 1] for i in unconnected_layers]
    else:
        # OpenCV 3.x or other configurations
        self.__out_layers = [self.__out_layers[i[0] - 1] for i in unconnected_layers]
  ...
  def predict(self, img=None, size=(416, 416), thresh=0.35, nms_threshold=0.6) -> list:                               # CHANGED THRESHOLD VALUE; thresh = 0.35, nms_threshold=0.6
    """
    ...
    for output in self.__net.forward(self.__out_layers):
      for detection in output:
        ...
        if confidence > thresh:
          print("classID:", classID, " | total labels:", len(self.__labels))
    ...
    #print(detections)                                                          # ADDED PRINT STATEMENT
    return detections